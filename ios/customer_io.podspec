...
Pod::Spec.new do |s|
  ...
  #
  # Declared on the standalone pod, unlike the umbrella subspecs above, because the released
  # `CustomerIO` umbrella has no MessagingInbox subspec — the sample Podfile branch-overrides the
  # standalone CIO pods but not the umbrella, so the subspec form cannot resolve and pod install
  # fails with "None of your spec sources contain a spec satisfying CustomerIO/MessagingInbox".
  # TODO(inbox-release): the subspec is being added to CustomerIO.podspec on the native branch; this
  # can move to `CustomerIO/MessagingInbox` for consistency once that ships.
  s.dependency "CustomerIOMessagingInbox", native_sdk_version
