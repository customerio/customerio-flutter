...
function main() {
	cat <<EOF
===============================================================================
flutter-sample-spm setup checklist

Follow the steps in to setup dev environment to be able to build and run the
flutter-sample-spm app in the simulator or on a phone.
...
- Follow prompts to get virtual device setup'
	...
	step 'Clone the "flutter-sample-spm" repository by running:
	git clone git@github.com:customerio/flutter-sample-spm.git' run_clone_repo

	step 'Download google credentials file and save it for android project
- Open 1Password
- Search for "Ami App Flutter - google-services.json"
- Download the file and save in the "flutter-sample-spm/android/app" directory'

	step 'Download google credentials file and save it for ios project
- Open 1Password
- Search for "Ami App Flutter - GoogleService-Info.plist"
- Download the file and save in the "flutter-sample-spm/ios/Runner" directory'
...
You can also save this in your ~/.zshrc file so that you do not need to set it again.'
	...
	step 'Navigate into the "flutter-sample-spm" repository'
...
function run_clone_repo() {
	...
	repo_dir="$dir/flutter-sample-spm"
	if [[ ! -d "$repo_dir" ]] ; then
	  git clone git@github.com:customerio/flutter-sample-spm.git $repo_dir
	fi
}
